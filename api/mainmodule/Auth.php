        ...
        try {
            ...
            if ($stmt->rowCount() > 0) {
                ...
                if ($this->check_password($pword, $res['password'])) {
                    ...
                    $email = $res['email'];
                    $fourpinpwd = $res['fourpinpwd'];
                    $sixpinpwd = $res['sixpinpwd'];
                    $token = $this->generate_token($res['users_id']);

                    $code = 200;
                    $remarks = "success";
                    $message = "Logged in successfully.";
                    $payload = array(
                        "user_id" => $id, "username" => $username, "token" => $token, "email" => $email, "fourpinpwd" => $fourpinpwd, "sixpinpwd" => $sixpinpwd);
